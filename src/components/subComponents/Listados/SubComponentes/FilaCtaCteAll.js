...
const FilaCtaCte = ({ id, item }) => {
    ...
    return (
        <tr key={id}>
            <td style={{ textAlign: 'center' }}>{moment(item.fecha).format('DD/MM/YYYY HH:MM')} Hs</td>
            <td style={{ textAlign: 'center' }}>
                {item.razsoc} ({item.ndoc})
            </td>
                ...
                <span
                    style={
                        parseFloat(item.importe) > 0
                            ? { marginRight: '20px', fontSize: '16px', color: 'green', fontWeight: 'bold' }
                            : { marginRight: '20px', fontSize: '16px', color: 'red', fontWeight: 'bold' }
                    }
                >
