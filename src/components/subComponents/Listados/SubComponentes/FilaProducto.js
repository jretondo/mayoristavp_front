...
const FilaProducto = ({
    ...
    const EliminarOff = async (e, id, name, primero, pagina) => {
        ...
        swal({
            ...
            if (willDelete) {
                setEsperar(true);
                await axios
                    .delete(`${UrlNodeServer.productsDir.products}/${id}`, {
                        headers: {
    ...
    const toggleProducto = (e) => {
        e.preventDefault();
        setEsperar(true);
        axios
            .put(
                `${UrlNodeServer.productsDir.products}/toggle/${item.id_prod}`,
            ...
            .then((res) => {
                ...
                if (status === 200) {
                    setActividadStr(
                        'El usuario ha ' +
                            (item.enabled === 1 ? 'desactivado' : 'activado') +
                            " el producto '" +
                            item.name +
                            "'",
                    );
                    setNvaActCall(!nvaActCall);
                    setMsgStrong('Producto ' + (item.enabled === 1 ? 'desactivado' : 'activado') + ' con éxito!');
                    setMsgGralAlert('');
                    setSuccessAlert(true);
                    setAlertar(!alertar);
                    setCall(!call);
                    ...
                    setMsgStrong('Hubo un error! ');
                    setMsgGralAlert('No se pudo ' + (item.enabled === 1 ? 'desactivar' : 'activar') + ' el producto.');
                    setSuccessAlert(false);
                    setAlertar(!alertar);
                }
            })
    ...
    const updatePriceCompra = async () => {
        ...
        await axios
            .put(`${UrlNodeServer.productsDir.sub.cost}/${item.id_prod}`, data, {
                headers: {
                    Authorization: 'Bearer ' + localStorage.getItem('user-token'),
                },
            })
    ...
    return (
        <>
            <tr key={id}>
                        ...
                        <Media>
                            <a className="avatar rounded-circle mr-3" href="#pablo" onClick={(e) => e.preventDefault()}>
                                <img
                                    alt="..."
                                    src={
                                        UrlNodeServer.publicFolder.prodImages +
                                        (item.url_img ? item.url_img : 'product.png')
                                    }
                ...
                <td style={{ textAlign: 'center' }}>{item.family}</td>
                <td style={{ textAlign: 'center' }} onDoubleClick={() => actChangePrice()}>
                    {compraBool ? (
                        <input
                            id="inpNewCostTxt"
                            value={newCompra}
                            onChange={(e) => setNewCompra(e.target.value)}
                            onKeyDown={(e) => changeNewCompra(e)}
                        />
                    ) : (
                ...
                <td style={{ textAlign: 'center' }}>
                    <span className="badge badge-dot">
                        <i
                            className={parseInt(item.enabled) === 1 ? 'bg-success' : 'bg-danger'}
                            style={{ width: '15px', height: '15px' }}
                        />
                    </span>
                </td>
                <td style={{ textAlign: 'center' }}>{'$ ' + formatMoney(item.vta_price) + ' (' + item.porc_minor}%)</td>
                <td className="text-right">
                    <UncontrolledDropdown>
                        ...
                        <DropdownMenu className="dropdown-menu-arrow" right>
                            <DropdownItem href="#pablo" onClick={(e) => NuevoStock(e)}>
                            ...
                            >
                                ...
                                */}
                            <DropdownItem href="#pablo" onClick={(e) => VerDetalles(e, item.id_prod)}>
                            ...
                            <DropdownItem href="#pablo" onClick={(e) => copiarDetalle(e, item.id_prod)}>
                                <i className="fas fa-copy"></i>
                                Copiar Producto
                            </DropdownItem>
                            <DropdownItem href="#pablo" onClick={(e) => toggleProducto(e, item.id_prod)}>
                                <i className="fas fa-copy"></i>
                                {item.enabled === 1 ? 'Desactivar' : 'Activar'}
                            </DropdownItem>
                        </DropdownMenu>
                    </UncontrolledDropdown>
                </td>
            ...
            <ModalMoverStock
                ...
                call={call}
            />
            <ModalChangeCodBarras setModal={setModal3} modal={modal3} item={item} setCall={setCall} call={call} />
        </>
    );
};
