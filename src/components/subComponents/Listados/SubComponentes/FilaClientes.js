    ...
    const EliminarOff = async (e, id, name, primero, pagina) => {
        ...
        swal({
            ...
            if (willDelete) {
                setEsperar(true);
                await axios
                    .delete(`${UrlNodeServer.clientesDir.clientes}/${id}`, {
                        headers: {
        ...
        <tr key={id}>
            ...
            <td style={{ textAlign: 'center' }}>
                {parseInt(item.cond_iva) === 1
                    ? 'Res. Inscripto'
                    : parseInt(item.cond_iva) === 6
                    ? 'Monotributista'
                    : parseInt(item.cond_iva) === 4
                    ? 'Exento'
                    : 'Cons. Final'}
            </td>
            <td className="text-right">
                <UncontrolledDropdown>
                    ...
                    <DropdownMenu className="dropdown-menu-arrow" right>
                        ...
                        <DropdownItem href="#pablo" onClick={(e) => VerCtaCte(e, item.id, item.razsoc)}>
